...
class Data():
    ...
    def __init__(self,args):
        ...
        # print('DataSet samples 10X10: ', self.data[0][0:10][0:10])
        #print('DataSet range is from %s to %s' % (np.min(self.data), np.max(self.data)))
        self.data_init()

    '''
     ---------------------将整个数据集划分3×3的邻域作为SAE的输入-----------------------------------------
    '''
    ...
    def data_init(self):
        ...
        sio.savemat(os.path.join(self.result,'info.mat'),{
            ...
            'class_num_deprecated':np.max(self.data_gt),
            'class_num': np.max(self.data_gt) if np.min(self.data_gt) > 0 else np.max(self.data_gt) + 1
        })
