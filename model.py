...
class Model():
    '''
    定义Model类的结构体，包括数据集/维度/路径/类别数等参数
    '''
    def __init__(self,args,sess):
        ...
        self.dim = info['dim']
        self.class_num = int(info['class_num'])  #  + 1  # self.class_num = 13
        ...
        if args.use_lr_decay:
            ...
            self.lr = args.lr
        '''
        第一个创新点：将聚类损失，分类损失都加入到总损失中，即引入无监督信息指导模型正则
        '''
    ...
    '''
    定义损失函数，损失函数由两部分部分组成，分别是：有标签数据的预测值与真实值（分类损失）、聚类标签与预测值（聚类损失）
    损失函数都由交叉熵给出
    最后总的损失函数是两个交叉熵的和
    '''
    def loss(self):
    ...
    def train(self,dataset):
        ...
        for i in range(1,self.epoch):
            ...
            # print("train_data.shape, train_label.shape, train_p_label.shape = ", train_data.shape, train_label.shape, train_p_label.shape)
            # print("train_label = ", train_label)
            # print("train_p_label = ", train_p_label)
            #输出训练标签和聚类标签
            # train_data, train_label = train_data,train_label - 1
    ...
    def test2(self,dataset):
        ...
        try:
            while True:
                ...
                """ 2020-01-24 修改 """
                # test_data, test_label = test_data, test_label - 1
    ...
    def save_decode_map(self,dataset):
        ...
        plt.savefig(os.path.join(self.result, 'decode_map_' + self.data_name +str(self.train_num) + '.png'), format='png', dpi=800)  # bbox_inches='tight',pad_inches=0)
        plt.close()
    ...
    def save_decode_seg_map(self,dataset):
        ...
        plt.close()
        print('best aa:0.97252')
        print('best oa:0.96942')
        print('best kappa:0.97827')
